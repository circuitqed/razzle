...
def save_game(
    ...
    with get_connection(db_path) as conn:
        conn.execute("""
            INSERT INTO games (game_id, player1_type, player2_type, ai_simulations, state_json, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(game_id) DO UPDATE SET
                state_json = excluded.state_json,
                updated_at = excluded.updated_at
        """, (game_id, player1_type, player2_type, ai_simulations, state_json, now, now))
        conn.commit()
...
def load_game(game_id: str, db_path: Path = DEFAULT_DB_PATH) -> Optional[dict]:
    ...
    Returns dict with keys: game_id, player1_type, player2_type, ai_simulations, state
